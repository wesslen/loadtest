...
def save_results_to_csv(results, filename):
    headers = ['Endpoint', 'Request Type', 'Payload Size', 'Concurrency', 'Duration', 'Creations', 'Failures']
...
@app.command()
def main(
    design_type: str = typer.Option("full", help="The design type of the test matrix: 'full' or 'fractional'."),
    fraction: float = typer.Option(None, help="The fraction of the test matrix to use, required only if design_type is 'fractional'.")
):
    """
    ...
    final_matrix = test_matrix if design_type == 'full' else fractional_design(test_matrix, fraction)

    results = []
    load_tester = LoadTester()

    for endpoint, request_type, payload_size, concurrency in final_matrix:
        duration, creations, failures = load_tester.run_test(endpoint, request_type, payload_size, concurrency)
        results.append([endpoint, request_type, payload_size, concurrency, duration, creations, failures])
